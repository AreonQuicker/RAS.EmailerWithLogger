...
namespace RAS.EmailerWithLogger.Utils
{
    internal static class Utils
    {
        internal static async Task CreateSchemaAndTableAsync(IDataAccessQuery dataAccessQuery)
        {
            if (dataAccessQuery != null)
            {
                ...
                if (schemaCount == 0)
                {
                    await dataAccessQuery.FirstOrDefaultQueryAsync<dynamic>("CREATE SCHEMA Emailer");
                }
